...
class LiquidityPoolAsset {
   ...
   */
  toXDRObject() {
    const lpConstantProductParamsXdr = new curr_generated.default.LiquidityPoolConstantProductParameters({
      assetA: this.assetA.toXDRObject(),
      assetB: this.assetB.toXDRObject(),
      fee: this.fee
    });
    const lpParamsXdr = curr_generated.default.LiquidityPoolParameters.liquidityPoolConstantProduct(
      lpConstantProductParamsXdr
    );
    return curr_generated.default.ChangeTrustAsset.assetTypePoolShare(lpParamsXdr);
  }
  /**
  ...
  toString() {
    const poolId = get_liquidity_pool_id.getLiquidityPoolId(
      "constant_product",
