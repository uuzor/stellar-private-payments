...
class CallBuilder {
  ...
  stream(options = {}) {
    ...
    streamUrl.searchParams.set("X-Client-Version", horizon_axios_client.version);
    ...
    const createEventSource = () => {
      try {
        es = new eventsource.EventSource(streamUrl.toString());
      } catch (err) {
        if (options.onerror) {
          options.onerror(err);
        }
  ...
  checkFilter() {
    if (this.filter.length >= 2) {
      throw new bad_request.BadRequestError("Too many filters specified", this.filter);
    }
  ...
  _requestFnForLink(link) {
    return async (opts = {}) => {
      let uri;
      if (link.templated) {
        uri = new URL(url.expandUriTemplate(link.href, opts), this.url);
      } else {
