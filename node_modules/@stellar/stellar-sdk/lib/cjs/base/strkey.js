...
var checksum = require('./util/checksum.js');

function _interopDefault (e) { return e && e.__esModule ? e : { default: e }; }

var base32__default = /*#__PURE__*/_interopDefault(base32);
...
function decodeCheck(versionByteName, encoded) {
  ...
  const decoded = base32__default.default.decode(encoded);
  const versionByte = decoded[0];
  const payload = decoded.slice(0, -2);
  const data = payload.slice(1);
  const checksum$1 = decoded.slice(-2);
  if (encoded !== base32__default.default.encode(decoded)) {
  ...
  if (!checksum.verifyChecksum(expectedChecksum, checksum$1)) {
...
function encodeCheck(versionByteName, data) {
  ...
  const versionBuffer = buffer.Buffer.from([versionByte]);
  const payload = buffer.Buffer.concat([versionBuffer, data]);
  const checksum = buffer.Buffer.from(calculateChecksum(payload));
  const unencoded = buffer.Buffer.concat([payload, checksum]);
  return base32__default.default.encode(unencoded);
}
