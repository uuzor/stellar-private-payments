...
import { Hyper } from '../node_modules/.pnpm/@stellar_js-xdr@4.0.0/node_modules/@stellar/js-xdr/src/hyper.js';
...
import { extractBaseAddress, decodeAddressToMuxedAccount } from './util/decode_encode_muxed_account.js';
...
const HYPER_MAX_VALUE = Hyper.MAX_VALUE;
...
class TransactionBuilder {
  ...
  build() {
    ...
    const fee = new BigNumber(this.baseFee).times(this.operations.length).toNumber();
    if (fee > UINT32_MAX) {
      throw new Error(
    ...
    if (this.sorobanData) {
      ...
      attrs.fee = new BigNumber(attrs.fee).plus(this.sorobanData.resourceFee().toString()).toNumber();
  ...
  static buildFeeBumpTransaction(feeSource, baseFee, innerTx, networkPassphrase) {
    ...
    const innerInclusionFee = new BigNumber(innerTx.fee).minus(resourceFee).div(innerOps);
...
export { BASE_FEE, TimeoutInfinite, TransactionBuilder, isValidDate };
